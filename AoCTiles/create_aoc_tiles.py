...
YEAR_CONFIGS = {
    ...
    2020: {
        "year_folder_to_days": "src/solutions",
...
def get_solution_paths_dict_for_years() -> dict[int, dict[int, list[str]]]:
    ...
    for year_dir in sorted(
        ...
        for days in get_paths_matching_regex(
            ...
            solutions = [s for s in solutions if s.suffix in config["extensions"]]
...
def parse_leaderboard(html: str) -> dict[int, DayScores]:
...
def request_leaderboard(year: int) -> dict[int, DayScores]:
    with open(SESSION_COOKIE_PATH) as cookie_file:
        ...
        data = requests.get(
            ...
            headers={
                "User-Agent": "jenarvaezg@gmail.com",
            },
        ).text
